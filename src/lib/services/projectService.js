  ...
  try {
    ...
    try {
      ...
      if (Array.isArray(empResponse.data)) {
        ...
        Array.isArray(empResponse.data.data)
      ) {
    ...
    const processedProjects = projects.map((project) => {
      ...
      if (leaderId) {
        const leader = employees.find(
          (emp) => emp.id === leaderId || emp.emp_id === leaderId
        );
        if (leader) {
      ...
      const endDate =
        project.endDate || project.end_date
          ? new Date(project.endDate || project.end_date)
          : null;
...
export async function getEmployeeProjects(employeeId) {
  try {
    const res = await api.get(`/employee-projects/employees/${employeeId}`);
    return res.data.data || [];
  } catch (err) {
    console.error(`获取员工ID=${employeeId}的项目经历失败:`, err);
    throw err;
  }
}
    ...
    if (leaderId) {
      try {
        ...
        if (empResponse.data && empResponse.data.data) {
          leaderName =
            empResponse.data.data.name || empResponse.data.data.emp_name || "";
  ...
  try {
    ...
    try {
      ...
      if (Array.isArray(projResponse.data)) {
        ...
        projResponse.data &&
        projResponse.data.data &&
        Array.isArray(projResponse.data.data)
      ) {
    ...
    const processedEmployeeProjects = employeeProjects.map((ep) => {
      ...
      if (empId) {
        const employee = employees.find(
          (emp) => emp.id === empId || emp.emp_id === empId
        );
        if (employee) {
      ...
      if (proId) {
        const project = projects.find(
          (proj) => proj.id === proId || proj.pro_id === proId
        );
        if (project) {
          projectName =
      ...
      if (approverId) {
        const approver = employees.find(
          (emp) => emp.id === approverId || emp.emp_id === approverId
        );
        if (approver) {
      ...
      return {
        ...
        role: ep.role || "",
        professionalAbility:
          ep.professionalAbility || ep.professional_ability || 0,
        managementAbility: ep.managementAbility || ep.management_ability || 0,
        cooperationAbility:
          ep.cooperationAbility || ep.cooperation_ability || 0,
    ...
    console.log(
      "处理后的员工项目经历数据:",
      processedEmployeeProjects.length,
      "条记录"
    );
  ...
  try {
    ...
    try {
      ...
      if (apiError.response && apiError.response.status === 404) {
        ...
        try {
          const backupResponse = await api.post(
            "/employee-projects",
            backupData
          );
...
export const updateEmployeeProject = async (id, employeeProjectData) => {
  try {
    console.log(
      "更新员工项目经历，前端提交数据:",
      JSON.stringify(employeeProjectData, null, 2)
    );
    ...
    try {
      ...
      if (apiError.response && apiError.response.status === 404) {
        ...
        try {
          const backupResponse = await api.put(
