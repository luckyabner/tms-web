...
export async function getEmployeePerformances(employeeId) {
  try {
    const res = await api.get(`/employee-performances/employees/${employeeId}`);
    return res.data.data || [];
  } catch (err) {
    console.error(`获取员工ID=${employeeId}绩效考核列表失败:`, err);
    // 返回空数组，确保前端流程不中断
    return [];
  }
}
...
const processPerformanceData = (performances) => {
  ...
  console.log(
    "每条记录的ID:",
    processedPerformances.map((p) => p.id)
  );
...
const getMockPerformances = () => {
  ...
  return [
    ...
    },
    {
      ...
      endDate: formatDate(
        new Date(threeMonthsLater.getTime() + 90 * 24 * 60 * 60 * 1000)
      ),
      end_date: formatDate(
        new Date(threeMonthsLater.getTime() + 90 * 24 * 60 * 60 * 1000)
      ),
...
export const getPerformanceById = async (id) => {
  try {
    ...
    const processedPerf = {
      ...
      name:
        performance.name || performance.perName || performance.per_name || "",
...
const determinePerformanceState = (performance) => {
  ...
  const startDate = new Date(
    performance.startDate || performance.start_date || ""
  );
...
export const createPerformance = async (performanceData) => {
  try {
    ...
    const apiData = {
      ...
      startDate:
        performanceData.startDate || new Date().toISOString().split("T")[0],
      endDate:
...
export const getAllEmployeePerformances = async () => {
  try {
    ...
    try {
      ...
      if (empResponse.data && empResponse.data.data) {
        if (Array.isArray(empResponse.data.data)) {
          ...
          empResponse.data.data.records &&
          Array.isArray(empResponse.data.data.records)
        ) {
      ...
      try {
        ...
        if (
          empDepResponse.data &&
          empDepResponse.data.data &&
          Array.isArray(empDepResponse.data.data)
        ) {
    ...
    try {
      ...
      if (
        perfResponse.data &&
        perfResponse.data.data &&
        Array.isArray(perfResponse.data.data)
      ) {
    ...
    const processedEmpPerformances = empPerformances.map((empPerf) => {
      ...
      if ((!department || !position) && employeeDepartments.length > 0) {
        ...
        const empDep = employeeDepartments.find(
          (ed) =>
        ...
        if (empDep) {
          ...
          if (departments.length > 0) {
            const dept = departments.find(
              (d) => d.id === empDep.depId || d.dep_id === empDep.depId
            );
            if (dept) {
      ...
      const performance = performances.find(
        (p) => p.id === perId || p.per_id === perId
      );

      // 查找关联的评估人信息
      const approver = employees.find(
        (e) => e.id === approverId || e.emp_id === approverId
      );
      ...
      return {
        ...
        employeeName: employee
          ? employee.name || employee.emp_name || "未知员工"
          : "未知员工",
        ...
        performanceName: performance
          ? performance.name || performance.per_name || "未知考核"
          : "未知考核",
        approverId: approverId,
        approverName: approver
          ? approver.name || approver.emp_name || "未知评估人"
          : "未知评估人",
        score: empPerf.score || "-",
        state:
          empPerf.state === 1 ||
          empPerf.state === "1" ||
          empPerf.state === "completed" ||
          empPerf.state === "已完成"
            ? "已完成"
            : "未完成",
        description: empPerf.description || "",
        startDate: performance
          ? performance.startDate || performance.start_date || ""
          : "",
        endDate: performance
          ? performance.endDate || performance.end_date || ""
          : "",
    ...
    console.log(
      "处理后的员工绩效评估数据:",
      processedEmpPerformances.length,
      "条记录"
    );
    ...
    return getMockEmployeePerformances().map((item) => ({
      ...item,
      performanceName:
        getMockPerformances()[0].per_name || "2024年第一季度绩效考核",
    }));
  }
};
  ...
  try {
    ...
    try {
      const response = await api.put(
        `/employee-performances/${numericId}`,
        apiData
      );
...
const getMockEmployeePerformances = () => {
  ...
  return [
    ...
    },
    {
      ...
      state: "已完成",
      description:
        "人员招聘与培训工作出色，员工满意度提升，绩效管理体系优化明显",
      startDate: formatDate(oneMonthAgo),
      endDate: formatDate(today),
      createdAt: formatDate(oneMonthAgo),
    },
    ...
    },
    {
      ...
      state: "已完成",
      description:
        "产品规划清晰，用户需求分析到位，产品设计创新，团队协作能力强",
      startDate: formatDate(oneMonthAgo),
      endDate: formatDate(today),
      createdAt: formatDate(oneMonthAgo),
    },
