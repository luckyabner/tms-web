...
import { Briefcase, Plus, Users, Search, MoreHorizontal, Pencil, Trash2, User, Loader2, Calendar, CheckCircle2, Eye } from 'lucide-react';
    ...
    switch (status) {
      ...
      case '未开始':
        return {
          ...
          icon: <Briefcase className="h-4 w-4 text-amber-500 mr-1" />,
        };
      default:
        return {
          variant: 'outline',
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-blue-800 text-transparent bg-clip-text">项目管理</h1>
          <p className="text-muted-foreground">管理公司项目和团队分配</p>
        </div>
        <Button onClick={handleAddProject} className="bg-gradient-to-r from-blue-500 to-blue-700 hover:from-blue-600 hover:to-blue-800 text-white">
                ...
                {currentPageData.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={6} className="text-center py-10 text-muted-foreground">
                      {filteredProjects.length === 0 ? '暂无项目数据' : '没有匹配的搜索结果'}
                    </TableCell>
                  </TableRow>
                ) : (
                  currentPageData.map((project) => (
                    <TableRow key={project.id} className="hover:bg-gray-50">
                      ...
                      <TableCell>
                        <Badge variant={getStatusBadge(project.state).variant} className={getStatusBadge(project.state).className}>
